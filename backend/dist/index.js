...
// src/logger/backend.logger.ts
var import_pine = __toESM(require("pine"));
var logger = (0, import_pine.default)();
var BackendLogger = class {
  info(message, data = void 0) {
    logger.info(`${message}   ${void 0 != data ? JSON.stringify(data) : ""}`);
  }
  error(message) {
  ...
  routes() {
    this.express.post("/register", async (req, res) => {
      try {
        ...
        const encryptedPassword = await import_bcrypt.default.hash(password, 10);
        ...
        user.token = import_jsonwebtoken.default.sign(
...
var Mongoose = __toESM(require("mongoose"));
var import_dotenv = __toESM(require("dotenv"));
var console2 = __toESM(require("console"));
import_dotenv.default.config();
var connected;
var logger2 = new BackendLogger();
var connect2 = () => {
  const url = process.env.MONGO_CONNECTION_STRING;
  console2.log("from connect: process.env.MONGO_CONNECTION_STRING :::", process.env.MONGO_CONNECTION_STRING);
  if (connected) {
    return;
  }
  Mongoose.connect(url, {
    ...
    logger2.info("Connected to database", null);
    connected = true;
  }).catch((err) => {
...
var disconnect2 = () => {
  ...
  Mongoose.disconnect().then(() => {
    logger2.info("Disconnected from database", null);
    connected = false;
  }).catch((err) => {
    logger2.error(err.message);
  });
...
var logger3 = new BackendLogger();
server.on("error", (error) => {
  logger3.error(`Erreur: ${error.name}, Message:${error.message}`);
});
server.on("listening", function() {
  const addr = server.address();
  const bind = typeof addr === "string" ? `pipe ${addr}` : `port ${addr.port}`;
  logger3.info(`Listening on ${bind}`, null);
});
server.listen(port);
connect2();
setTimeout(() => {
